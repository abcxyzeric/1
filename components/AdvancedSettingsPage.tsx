...
const MODEL_OPTIONS = [
    { value: 'gemini-3-pro-preview', label: 'Gemini 3 Pro Preview (Mặc định, Thông minh nhất)' },
...
const AdvancedSettingsPage: React.FC = () => {
    ...
    const handleDeletePreset = (id: string) => {
        if (confirm('Bạn có chắc muốn xóa Preset này?')) {
            ...
            // Nếu preset đang được kích hoạt thì tự động bỏ kích hoạt nó (logic trong context chỉ lưu mảng ID, filter sẽ tự loại bỏ nếu ID không tồn tại nhưng tốt nhất nên clean up)
             // Context logic doesn't strictly auto-clean ID from active list if preset deleted, but it won't crash. 
        }
    };
            ...
            {activeTab === 'presets' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1 space-y-4">
                        ...
                        <div className="bg-gray-800/50 rounded-xl border border-gray-700/50 overflow-hidden max-h-[600px] overflow-y-auto">
                            {presets.map(preset => {
                                const isActive = activePresetIds.includes(preset.id);
                                return (
                                    <div 
                                        key={preset.id}
                                        className={`p-3 border-b border-gray-700/50 transition-colors ${editingPreset?.id === preset.id ? 'bg-gray-700/50' : 'hover:bg-gray-700/30'}`}
                                    >
                                        <div className="flex justify-between items-start cursor-pointer" onClick={() => setEditingPreset(preset)}>
                                            <div>
                                                <h3 className="font-semibold text-gray-200">{preset.name}</h3>
                                                <p className="text-xs text-gray-500 truncate max-w-[150px]">{preset.description || 'Không có mô tả'}</p>
                                            </div>
                                            {isActive && <CheckIcon className="w-4 h-4 text-green-400" />}
                                        </div>
                                        <div className="mt-3 flex gap-2 items-center justify-between">
                                            <label className="flex items-center gap-2 cursor-pointer select-none">
                                                <input 
                                                    type="checkbox"
                                                    checked={isActive}
                                                    onChange={() => toggleActivePreset(preset.id)}
                                                    className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-[var(--primary-600)] focus:ring-[var(--primary-500)]"
                                                />
                                                <span className={`text-xs px-2 py-0.5 rounded transition-colors ${isActive ? 'bg-green-900 text-green-300' : 'bg-gray-600 text-gray-300'}`}>
                                                    {isActive ? 'Đang bật' : 'Tắt'}
                                                </span>
                                            </label>
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleDeletePreset(preset.id); }}
                                                className="text-xs px-2 py-0.5 rounded bg-gray-700/50 text-red-400 hover:bg-red-900/50 hover:text-red-300"
                                            >
                                                Xóa
                                            </button>
                                        </div>
                        ...
                        <p className="text-xs text-gray-400 italic text-center">
                            Mẹo: Bạn có thể bật nhiều Preset cùng lúc để kết hợp các chỉ dẫn.
                        </p>
                    </div>
